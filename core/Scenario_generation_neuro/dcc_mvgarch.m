...
for i=1:k
    ...
    stdresid(:,i)=data(:,i)./sqrt(univariate{i}.ht);  % stdresid = Vt
end
...
%options=optimset('Algorithm', 'levenberg-marquardt','LargeScale','off','Display','iter',  'Diagnostics' , 'on','MaxFunEvals',20000,'MaxIter',1000);
options=optimset(options,'Algorithm',  'interior-point','LargeScale','off','Display','iter',  'Diagnostics' , 'on','MaxFunEvals',20000,'MaxIter',1000);
...
parameters=[];
H=zeros(t,k);   %[t,k]=size(data)
for i=1:k
    parameters=[parameters;univariate{i}.parameters];
    H(:,i)=univariate{i}.ht;          %方差
end
...
Ht=zeros(k,k,t);          %[t,k]=size(data)
stdresid=zeros(t,k);
Hstd=H.^(0.5);
for i=1:t
    Ht(:,:,i)=diag(Hstd(i,:))*Rt(:,:,i)*diag(Hstd(i,:));  %方差-协方差矩阵
    stdresid(i,:)=data(i,:)*Ht(:,:,i)^(-0.5);             %std = Vt
end
save tempHt Ht
clear Ht
