function [node_data,n_br,n_node,time,tree_mean,tree_sqvar,parsM,finaldata1]=b_scenario_generation_ARMA_GARCH_kmeans(stock_history_data,n_branch,stage,scenario)
t1=clock;
...
if n_sample(1)==n_branch(2)
    for i = 1:n_branch(2)
        pr(i,1)=1/n_sample(1);
    end
else
    for i = 1:n_branch(2)
        pr(i,1) = length(find(I==i))/n_sample(1);
    end
end
